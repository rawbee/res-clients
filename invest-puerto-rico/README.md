...
| `rfp/gather-assets/` | Source material from the principals | `Robby_Abaya_Resume.md` (**RFP-tailored Aug 13 — see D13**; earlier edits D7, D8) — this is the Appendix A attachment, not just source material. `hai-li.md` is Hai Tran's raw LinkedIn export, **not yet a resume** (G-39c); filename is stale, person is **Hai Tran** |
...
| `rfp/SALESFORCE-STUDY-GUIDE.html` | **Interactive Salesforce study guide for the principals** (written Aug 13, 2026). Thirteen modules ordered by the TECH-BRIEF's seven problems, written for engineers new to the platform: data model explorer, lead conversion, the HubSpot↔Salesforce seam with a sync-error triage tool, the attribution loop step by step, plus a term decoder, a "grill me" Q&A, a self-quiz, and a one-page cheat sheet. **Prep material, never submitted** | `open rfp/SALESFORCE-STUDY-GUIDE.html`. Also at <https://claude.ai/code/artifact/159bdc57-b2cd-4628-bf55-9144c5e36c4a>. Opens with the **D3 guardrail** — their stack is inferred and must never be asserted; our own Salesforce/HubSpot history is stated plainly |
...
| `rfp/DECK-COPY.md` | **Presentation-copy master for the deck** — slide-by-slide map of every string that exists only on the slides (headlines, divider words, Latin labels, footer taglines, exhibit captions), with `← PROPOSAL.md → §X` pointers for shared prose. No duplication of proposal content | Edit slide chrome here; edit substance in PROPOSAL.md; then ask an agent to **re-sync the deck** |
| `rfp/PROPOSAL-DECK.html` | **32-slide 16:9 presentation deck** in the tryres.ai brand language (see D10). Prints to widescreen PDF via ⌘P. Open `[G-##]` items render as blue markup; internal notes + draft bar are screen-only | `open rfp/PROPOSAL-DECK.html`. Also at <https://claude.ai/code/artifact/23ee5054-eb07-4943-833f-0c7b8bc10142>. **A rendering, not a source** — substance lives in PROPOSAL.md, slide chrome in DECK-COPY.md; re-sync manually after editing either (it does not auto-update) |
...
**D13 — Robby's resume tailored to this RFP (Aug 13, Robby's direction: "custom tailor my resume for the attachment").** The resume in `gather-assets/` was written for an AI/solutions-architect role; it now reads as a CRM-architecture, systems-integration, and marketing-technology resume, so that the required Appendix A attachment argues the same case as §A/§B/§D instead of a different one. Changes: added a positioning subtitle and email/phone contact line; rewrote the summary around CRM, integration, attribution, and enablement; replaced `TECHNICAL SKILLS & EXPERTISE` with an `AREAS OF EXPERTISE` block ordered by RFP scope (CRM & martech → data governance → integrations & middleware → attribution → enablement → engineering → AI last, demoted but kept); expanded the **Masterworks** entry with the HubSpot-canonical stack, Segment/Braze fan-out, and the in-house automation service doing SMS, ad-audience sync, and offline conversion feedback to Meta/Google; sharpened the **Zocdoc** entry's Salesforce and growth-team language; reframed **Pinwheel** around integration architecture and documentation; added a Platform experience line and a certifications placeholder. *Ground rules held:* **no date, title, or employer was changed** (D7/D8 stand untouched), no new accomplishment was invented — every added claim traces to `gather-assets/`, `hai-li.md`, or Robby's own description recorded in D9 — and the `[DRAFT]` marker on the Res AI entry stays until Robby expands it. The pre-tailoring version is in git (`f1b7e65`). *Consequence:* two things this exposes are now tracked — the resume carries `[G-04a]`/`[G-19b]` tokens that must be cleared before PDF export (G-39d), and **Hai has no resume at all**, only a raw LinkedIn export, which is now a visible asymmetry in a required attachment (G-39c).
...
- [x] **Robby's resume tailored to the RFP (Aug 13, D13)** — repositioned to CRM/integration/martech; no dates or titles changed. Still needs his Res AI bullet expanded, the Worthy/Res overlap confirmed, and the new Masterworks bullets verified (G-39)
- [ ] **Hai has no resume — only a raw LinkedIn export.** Appendix A requires one for every named person; a polished resume next to an unformatted paste is a scoring risk. Agent can draft it from `hai-li.md`, Hai must supply the CRM/martech detail (G-39c). Due Aug 21
- [ ] Produce the three Appendix B artifacts (G-14/G-40, path decided in D12) + finalize both resumes (G-39/G-39b/G-39c) + take-or-strike the HubSpot certs (G-19b–23b)
- [x] **Salesforce study guide written (Aug 13, `rfp/SALESFORCE-STUDY-GUIDE.html`)** — interview/finalist prep for the principals, prioritized by the TECH-BRIEF's seven problems. Not a deliverable; nothing in it changes the proposal
